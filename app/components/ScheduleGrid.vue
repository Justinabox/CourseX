<template>
  <div class="w-full h-full flex flex-col gap-2 overflow-hidden">
    <div v-if="!isCompact" class="w-full grid grid-cols-7 text-xs text-cx-text-muted select-none">
      <div v-for="(d, di) in dayLabels" :key="di" class="text-center">{{ d }}</div>
    </div>
    <div class="w-full grow grid overflow-hidden" :class="isCompact ? 'grid-cols-1 gap-4' : 'grid-cols-7'">
      <div
        v-for="(dayIndex, colIdx) in dayIndices"
        :key="'col-' + dayIndex"
        class="flex flex-col"
        :class="isCompact ? 'gap-2' : ''"
      >
        <div v-if="isCompact" class="text-xs font-semibold text-cx-text-muted px-2">
          {{ dayLabels[dayIndex] }}
        </div>
        <div
          :class="[
            'relative border-cx-border bg-cx-surface-950/20 overflow-hidden',
            isCompact ? 'border rounded-lg' : 'border-r last:border-r-0',
            isCompact || colIdx === 0 ? 'pl-8' : '',
            isCompact ? 'h-[240px] sm:h-[280px]' : 'h-full'
          ]"
        ...
        >
          ...
          <div v-if="showTimeLabels(colIdx)" class="absolute inset-0 pointer-events-none">
...
const props = defineProps<{ blocks: ScheduleBlock[]; previewBlocks: ScheduleBlock[]; onBlockClick?: (id: string) => void; onDayMouseDown?: (e: MouseEvent, dayIndex: number) => void; compact?: boolean }>()
...
const showTimeLabels = (colIdx: number) => isCompact.value || colIdx === 0
